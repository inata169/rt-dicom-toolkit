...
"%TRUSTED_POWERSHELL%" -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command ^
  "$ErrorActionPreference='Stop';$root=[IO.Path]::GetFullPath($env:RTDT_BUNDLE_ROOT);$prefix=$root.TrimEnd([IO.Path]::DirectorySeparatorChar)+[IO.Path]::DirectorySeparatorChar;$count=0;$lines=Get-Content -LiteralPath (Join-Path $root 'SHA256SUMS.txt') -Encoding UTF8;foreach($line in $lines){if([string]::IsNullOrWhiteSpace($line)){continue};if($line -notmatch '^([0-9a-f]{64}) \*(.+)$'){throw ('Invalid checksum line: '+$line)};$expected=$matches[1];$relative=$matches[2].Replace('/',[IO.Path]::DirectorySeparatorChar);if([IO.Path]::IsPathRooted($relative)-or $relative.Contains(':')){throw ('Unsafe checksum path: '+$relative)};$full=[IO.Path]::GetFullPath((Join-Path $root $relative));if(-not $full.StartsWith($prefix,[StringComparison]::OrdinalIgnoreCase)){throw ('Checksum path escapes bundle: '+$relative)};if(-not [IO.File]::Exists($full)){throw ('Missing bundle file: '+$relative)};$actual=(Get-FileHash -LiteralPath $full -Algorithm SHA256).Hash.ToLowerInvariant();if($actual -ne $expected){throw ('SHA-256 mismatch: '+$relative)};$count++};if($count -eq 0){throw 'Checksum inventory is empty'};Write-Host ('Verified '+$count+' files.')"
if errorlevel 1 (
